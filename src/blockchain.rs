...
fn is_work_proven(
    ...
    let hash_prefix = hash.get(0..difficulty).expect("Error when encoding hash");
...
fn is_valid_chain(blockchain: &BlockChain) -> Result<(), BlockChainError> {
    ...
    if chain_len == 0 {
        ...
        let rest_block = BlockChain {
            blocks: blockchain.blocks.split_at(1).1.to_vec(),
        };
